...
def main():
    ...
    if threespace: # three dimensions
        s_x0 = x0[0].__float__() + 0.1
        s_x1 = x0[1].__float__() + 0.0
    ...
    for k in range(n_timesteps+1):
        if threespace:
            cost += obstacle_cost_weight * sum1(exp(-1*((X[k,:3].T - vertcat(sphere[0], sphere[1], sphere[2]))**2))/sphere[3])
        else:
            cost += obstacle_cost_weight * sum1(exp(-1*((X[k,:2].T - vertcat(circle[0], circle[1]))**2))/circle[2])
